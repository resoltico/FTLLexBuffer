...
from ftllexbuffer.runtime.functions import datetime_format, number_format
...
class TestNumberFormatParametricCombinations:
    ...
    def test_number_format_precision_combinations(
        ...
        if expected_decimal_digits == 0:
            # Should not have decimal separator (or only in thousands)
            # Check that if decimal_sep exists, it's not for decimals
            if decimal_sep in result:
                # Could be thousands separator, check position
                # This is fuzzy, just verify it's a string
                assert isinstance(result, str)
        # Should have decimal separator with right number of digits
        elif decimal_sep in result:
...
class TestDatetimeFormatStringConversions:
    ...
    def test_datetime_format_iso_strings(self, iso_string):
        ...
        assert isinstance(result, str)
        # Should not be error marker if valid ISO
        if "?" not in result:
            assert len(result) > 0
