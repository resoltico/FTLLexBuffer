...
class TestFluentBundleAddFunction:
    ...
    def test_add_function_registers_custom_function(self, bundle: Any) -> None:
        ...
        # Function is registered (internal state, hard to test directly)
        # Will be tested via format_pattern in integration tests
        assert True  # Function registration doesn't crash

    def test_add_function_with_callable(self, bundle: Any) -> None:
        """add_function accepts any callable."""

        bundle.add_function("LAMBDA", lambda x: x * 2)

        assert True  # Callable registered successfully
...
class TestFluentBundleMockedErrors:
    ...
    def test_format_pattern_with_keyerror_exception(self) -> None:
        ...
        with patch("ftllexbuffer.runtime.bundle.FluentResolver") as MockResolver:
            mock_resolver = Mock()
