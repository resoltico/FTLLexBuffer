...
from tests.strategies import camel_case_identifiers, snake_case_identifiers
...
class TestParameterNameConversion:
    ...
    def test_snake_to_camel_roundtrip(self, snake_name: str) -> None:
        """Property: snake → camel → snake preserves original (with caveats)."""
        registry = FunctionRegistry()

        camel = registry._to_camel_case(snake_name)
        reconstructed = registry._to_snake_case(camel)

        assert isinstance(camel, str), "Conversion must return string"
        assert isinstance(reconstructed, str), "Reverse conversion must return string"
    ...
    @given(camel_name=camel_case_identifiers())
    @settings(max_examples=500)
    def test_camel_to_snake_roundtrip(self, camel_name: str) -> None:
        """Property: camel → snake conversion produces valid snake_case."""
        registry = FunctionRegistry()

        snake = registry._to_snake_case(camel_name)

        assert isinstance(snake, str), "Conversion must return string"
        assert snake.islower() or "_" in snake, "Result should be lowercase or snake_case"
    ...
    def test_single_word_unchanged_by_conversion(self, word: str) -> None:
        """Property: Single words (no underscores/caps) unchanged by conversion."""
        registry = FunctionRegistry()

        camel = registry._to_camel_case(word)
        snake = registry._to_snake_case(word)

        assert camel == word, "Single lowercase word should not change in camelCase"
        assert snake == word, "Single lowercase word should not change in snake_case"
...
class TestConversionEdgeCases:
    ...
    @given(
        consecutive_caps=st.integers(min_value=2, max_value=5),
    )
    @settings(max_examples=100)
    def test_consecutive_capitals_handling(self, consecutive_caps: int) -> None:
        """Property: Consecutive capital letters handled consistently."""
        registry = FunctionRegistry()

        camel_with_caps = "a" + "B" * consecutive_caps + "c"
        snake = registry._to_snake_case(camel_with_caps)

        assert isinstance(snake, str), "Conversion must succeed"
        assert snake[0] == "a", "First char should be preserved"
