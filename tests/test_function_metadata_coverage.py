...
class TestFunctionMetadataIntegration:
    ...
    def test_should_inject_locale_with_built_in_registry(self) -> None:
        ...
        from ftllexbuffer.runtime.functions import FUNCTION_REGISTRY  # noqa: PLC0415

        # Built-in functions should inject locale
        assert should_inject_locale("NUMBER", FUNCTION_REGISTRY) is True
        assert should_inject_locale("DATETIME", FUNCTION_REGISTRY) is True
        assert should_inject_locale("CURRENCY", FUNCTION_REGISTRY) is True

        # Non-existent function shouldn't inject
        assert should_inject_locale("NONEXISTENT", FUNCTION_REGISTRY) is False
