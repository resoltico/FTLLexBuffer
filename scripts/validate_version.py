...
def get_runtime_version() -> str | None:
    ...
    try:
        import ftllexbuffer
        return ftllexbuffer.__version__
    except Exception:
        return None
...
def check_quick_reference_version(root: Path) -> CheckResult:
    ...
    try:
        ...
        # Look for version in footer pattern: **FTLLexBuffer Version**: X.Y.Z
        version_pattern = re.compile(
            r"\*\*FTLLexBuffer Version\*\*:\s*(\S+)"
        )
...
def detect_project_context(root: Path) -> tuple[bool, str]:
    ...
    # Check for FTLLexBuffer markers
...
def main() -> int:
    ...
    # Context detection
    is_ftllexbuffer, project_name = detect_project_context(root)

    if not is_ftllexbuffer:
        print("[SKIP] validate_version.py is for FTLLexBuffer project")
