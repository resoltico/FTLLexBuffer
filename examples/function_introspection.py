...
def demonstrate_registry_copy() -> None:
    """Demonstrate registry copying for isolation."""
    print("\n" + "=" * 70)
    print("Example 7: Registry Copying for Isolation")
    print("=" * 70)

    # Create registry with built-in functions
    from ftllexbuffer.runtime.functions import FUNCTION_REGISTRY

    print(f"\n[ORIGINAL] Original registry has {len(FUNCTION_REGISTRY)} functions")
    print(f"Functions: {FUNCTION_REGISTRY.list_functions()}")

    # Copy registry for customization
    print("\n[OPERATION] Creating copy for customization")
    custom_registry = FUNCTION_REGISTRY.copy()
    ...
    print(f"\n[VERIFICATION] Original registry still has {len(FUNCTION_REGISTRY)} functions")
    print(f"Functions: {FUNCTION_REGISTRY.list_functions()}")
    print(f"CUSTOM in original: {'CUSTOM' in FUNCTION_REGISTRY}")
